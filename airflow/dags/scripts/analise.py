...
def run_analysis():
    ...
    df_q1.to_sql("q1__quantidade_apontamentos_ususario",target_engine, index=False, if_exists='replace')

    df_q2 = df.groupby("company").count().id.reset_index()
    df_q2.to_sql("q2__quantidade_apontamentos_empresa",target_engine, index=False, if_exists='replace' )

    df_q3 = df_q1.sort_values("id", ascending=False)[:10]
    df_q3.to_sql("q3__top10_usuarios",target_engine, index=False, if_exists='replace')

    df_q4 = df.copy()
    df_q4["insert_date"] = df_q4["insert_date"].apply(lambda x: x.date())
    df_q4 = df_q4.groupby('insert_date').count().id.reset_index()
    df_q4.to_sql("q4__media_diaria",target_engine, index=False, if_exists='replace')
